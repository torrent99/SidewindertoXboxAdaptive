...
uint8_t
    ...
    hat_bits,                                   // SKS  hat bits to play with
    hat_val,                                   // SKS  hat bits to play with
    sw_repcopy[SW_REPSZ_3DP] ,			// USB report data copy to play with
...
static uint8_t sw_repchg ( void )
{
    ...
    sw_repcopy [4] = sw_report [4] | hat_val; // SKS force buttons 5-8 to on

    sw_report [4] = sw_repcopy [4];  // SKS force buttons 5-8 to on

    for ( ; i-- ; )
	if ( sw_repsav[i] != sw_report[i] )
	{
	    sw_repsav[i] = sw_report[i] ;
	    sw_repcopy[i] = sw_report[i] ;
	    r = TRUE ;
	}
    return ( r ) ;
}
...
int FA_NORETURN( main ) ( void )
{
	...
	if ( usb_configured() )
	{
		...
		LED_on() ;
						// Send report
		usb_send_IN( sw_repcopy, sw_reportsz ) ;
